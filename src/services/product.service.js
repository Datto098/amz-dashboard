...
const createProduct = async (data) => {
	...
	// Kiểm tra nếu product đã tồn tại trong DB
	const existingProduct = await Product.findOne({ asin: processedData.asin });
	if (existingProduct) {
...
const getUniqueSoldBy = async () => {
	const soldByArr = await Product.find({}, 'soldBy').lean();
	const soldBySet = new Set(soldByArr.map((p) => p.soldBy).filter(Boolean));
	return Array.from(soldBySet);
};

const getUniqueCategories = async () => {
	const categoryArr = await Product.find({}, 'category').lean();
